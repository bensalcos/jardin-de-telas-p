...
export default function UserEditForm({ usuario }) {
  ...
  const onSubmit = async (values) => {
    ...
    const getChangedFields = (current, original) => {
      if (typeof current === "object" && current !== null) {
        return Object.keys(current).reduce((acc, key) => {
          const changed = getChangedFields(current[key], original?.[key]);
          if (changed !== undefined && Object.keys(changed).length > 0) {
            acc[key] = changed;
          }
          return acc;
        }, {});
    ...
    const changedFields = getChangedFields(formattedValues, usuario);
    //console.log("Datos", session.accessToken, changedFields);
    ...
    try {
      const response = await fetch(`https://api.bensalcos.dev/api/v1/usuarios/${usuario.id}/`, {
        method: "PATCH",
        headers: {
        ...
        <Card className="p-4">
          <CardHeader>
            <CardTitle className="flex justify-center text-lg">
              Modificar datos de {usuario.nombre} {usuario.apellido}
            </CardTitle>
          </CardHeader>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="w-4/12 p-2">
                  ...
                  <FormControl>
                    <Input type="telefono" placeholder="+56912345678" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
